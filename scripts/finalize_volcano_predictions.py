...
SCRIPT_VERSION = "stage7_volcano_final_v2"
REVIEWER = "script:stage7_volcano_final_v2"
REPO_ROOT = Path(__file__).resolve().parent.parent


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Finalize volcano cohort inclusion/exclusion decisions for the scoped eruption slice.")
    ...
    parser.add_argument("--run-key", help="Unique Stage 7 volcano run key. Defaults to a timestamped key.")
    ...
    parser.add_argument(
        "--adjudications-path",
        default=str(REPO_ROOT / "data" / "volcano_final_adjudications.json"),
        help="Path to volcano final adjudication JSON keyed by report_number:candidate_seq.",
    )
...
def generate_run_key() -> str:
    return "stage7-volcano-final-" + datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
...
def decide_final_status(prediction: dict[str, Any], adjudications: dict[str, dict[str, Any]]) -> tuple[str, str, dict[str, Any]]:
    ...
    if prediction["match_status"] in {"exact_hit", "near_hit", "similar_only", "miss"}:
        return (
            ...
            "Prediction belongs to the scoped volcano slice and has a resolved match outcome.",
            {
                "script_version": SCRIPT_VERSION,
                "reason_code": "resolved_volcano_slice",
            },
        )

    raise RuntimeError(f"Missing volcano final adjudication rule for unresolved prediction {key}.")
...
def insert_reviews(cur, run_id: int, decisions: list[tuple[int, str, str, dict[str, Any]]]) -> None:
    params = [
        (prediction_id, run_id, "volcano", status, True, REVIEWER, rationale, Json(meta))
...
def main() -> int:
    ...
    try:
        with conn.cursor() as cur:
            stage4_run_id, resolved_stage4_run_key, stage4_filter = fetch_run(cur, "stage4_match_scoring", args.stage4_run_key, family="volcano")
        ...
        if run_id is not None:
            ...
            conn.commit()
        print(f"Stage 7 volcano run failed: {exc}", file=sys.stderr)
        return 1
    finally:
        conn.close()
