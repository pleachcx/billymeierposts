...
SCRIPT_VERSION = "storm_export_v1"
OUTPUT_ROOT = Path("data") / "exports" / "storm"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Export storm prediction analysis artifacts.")
    ...
    parser.add_argument("--output-dir", help="Output directory. Defaults to data/exports/storm/<stage4_run_key>.")
...
def resolve_run_set(cur, args: argparse.Namespace) -> RunSet:
    stage4 = fetch_run(cur, "stage4_match_scoring", args.stage4_run_key, family="storm")
    ...
    stage3 = fetch_run(cur, "stage3_event_ledger", stage3_run_key, family="storm") if stage3_run_key else None
    ...
    cur.execute(
        ...
        WHERE stage = 'stage5_probability_model'
          ...
          AND source_filter->>'family' = 'storm'
        ORDER BY created_at DESC
        LIMIT 1
        """,
        (stage4["run_key"],),
