...
SCRIPT_VERSION = "war_conflict_export_v1"
OUTPUT_ROOT = Path("data") / "exports" / "war_conflict"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Export war_conflict prediction analysis artifacts.")
    ...
    parser.add_argument("--output-dir", help="Output directory. Defaults to data/exports/war_conflict/<stage4_run_key>.")
...
def resolve_run_set(cur, args: argparse.Namespace) -> RunSet:
    stage4 = fetch_run(cur, "stage4_match_scoring", args.stage4_run_key, family="war_conflict")
    ...
    stage3 = fetch_run(cur, "stage3_event_ledger", stage3_run_key, family="war_conflict") if stage3_run_key else None
    ...
    cur.execute(
        ...
        WHERE stage = 'stage5_probability_model'
          ...
          AND source_filter->>'family' = 'war_conflict'
        ORDER BY created_at DESC
        LIMIT 1
        """,
        (stage4["run_key"],),
