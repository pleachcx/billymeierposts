...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Finalize epidemic cohort inclusion/exclusion decisions for the named-disease slice.")
    ...
    parser.add_argument("--run-key", help="Unique Stage 7 epidemic run key. Defaults to a timestamped key.")
    ...
    parser.add_argument(
        "--adjudications-path",
        default=str(REPO_ROOT / "data" / "epidemic_final_adjudications.json"),
        help="Path to manual epidemic adjudication rules keyed by report_number:candidate_seq.",
    )
...
def generate_run_key() -> str:
    return "stage7-epidemic-final-" + datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
...
def decide_final_status(prediction: dict[str, Any], adjudications: dict[str, dict[str, Any]]) -> tuple[str, str, dict[str, Any]]:
    if prediction["match_status"] in {"exact_hit", "near_hit", "similar_only", "miss"}:
        return (
            ...
            "Prediction belongs to the current named-disease epidemic slice and has a resolved match outcome.",
            {
                "script_version": SCRIPT_VERSION,
                "reason_code": "resolved_named_disease_slice",
            },
        )
...
def main() -> int:
    ...
    try:
        with conn.cursor() as cur:
            stage4_run_id, resolved_stage4_run_key, stage4_filter = fetch_run(cur, "stage4_match_scoring", args.stage4_run_key, family="epidemic")
        ...
        if not args.dry_run and run_id is not None:
            ...
            conn.commit()
        print(f"Stage 7 epidemic finalization failed: {exc}", file=sys.stderr)
        return 1
    finally:
        conn.close()
