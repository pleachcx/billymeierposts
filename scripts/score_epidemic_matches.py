...
SCRIPT_VERSION = "stage4_epidemic_named_disease_v1"
REVIEWER = "script:stage4_epidemic_named_disease_v1"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Stage 4 epidemic match scorer for the named-disease catalog slice.")
    ...
    parser.add_argument("--stage3-run-key", help="Stage 3 epidemic run key. Defaults to latest completed Stage 3 run.")
    parser.add_argument("--run-key", help="Unique Stage 4 epidemic run key. Defaults to a timestamped key.")
...
def generate_run_key() -> str:
    return "stage4-epidemic-" + datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
...
def insert_run(cur, run_key: str, source_filter: dict[str, Any], notes: str | None) -> int:
    cur.execute(
        ...
        (
            ...
            Json(source_filter),
            notes or None,
            Json({"family": "epidemic", "scope": "named_disease_catalog_v1"}),
        ),
    )
...
def best_row(rows: list[LedgerRow]) -> LedgerRow | None:
    ...
    return sorted(
        ...
        key=lambda row: (
            ...
            row.time_delta_days if row.time_delta_days is not None else math.inf,
...
def classify(rows: list[LedgerRow]) -> tuple[str, int | None, str, dict[str, Any]]:
    ...
    if exact:
        row = best_row(exact)
        return "exact_hit", row.event_ledger_id, "Exact epidemic event found in curated official catalog.", meta
    if near:
        row = best_row(near)
        return "near_hit", row.event_ledger_id, "Near-window epidemic event found in curated official catalog.", meta
    if log_only:
        row = best_row(log_only)
        return "similar_only", row.event_ledger_id, "Same-disease epidemic event found, but the event type or timing only partially matches.", meta
    return "miss", None, "No official catalog event matched the scoped epidemic prediction.", meta
...
def insert_reviews(cur, run_id: int, reviews: list[tuple[int, int | None, str, str, dict[str, Any]]]) -> None:
    params = [
        (prediction_id, event_ledger_id, run_id, status, True, REVIEWER, None, rationale, Json(meta))
        for prediction_id, event_ledger_id, status, rationale, meta in reviews
    ]
    execute_batch(
        cur,
        """
...
def main() -> int:
    ...
    try:
        ...
        for prediction in predictions:
            status, event_ledger_id, rationale, meta = classify(ledger_by_prediction.get(prediction.prediction_id, []))
            decisions.append((status, event_ledger_id, prediction.prediction_id))
            reviews.append((prediction.prediction_id, event_ledger_id, status, rationale, meta))
            counts[status] += 1

        if not args.dry_run and run_id is not None:
            with conn.cursor() as cur:
                demote_existing(cur, [prediction.prediction_id for prediction in predictions])
                update_predictions(cur, decisions)
                insert_reviews(cur, run_id, reviews)
                update_run(
                    cur,
                    run_id,
                    "completed",
        ...
        if not args.dry_run and run_id is not None:
            ...
            conn.commit()
        print(f"Stage 4 epidemic scoring failed: {exc}", file=sys.stderr)
        return 1
    finally:
        conn.close()
