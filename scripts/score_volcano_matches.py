...
SCRIPT_VERSION = "stage4_volcano_eruption_catalog_v1"
REVIEWER = "script:stage4_volcano_eruption_catalog_v1"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Stage 4 volcano match scorer for the curated eruption catalog slice.")
    ...
    parser.add_argument("--stage3-run-key", help="Stage 3 volcano run key. Defaults to latest completed Stage 3 volcano run.")
    parser.add_argument("--run-key", help="Unique Stage 4 volcano run key. Defaults to a timestamped key.")
    parser.add_argument("--notes", default="", help="Free-form run notes.")
    parser.add_argument(
        "--overrides-path",
        default=str(REPO_ROOT / "data" / "volcano_prediction_overrides.json"),
        help="Path to volcano prediction override JSON keyed by report_number:candidate_seq.",
    )
...
def generate_run_key() -> str:
    return "stage4-volcano-" + datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
...
def insert_run(cur, run_key: str, source_filter: dict[str, Any], notes: str | None) -> int:
    cur.execute(
        ...
        (
            ...
            Json(source_filter),
            notes or None,
            Json({"family": "volcano", "scope": "eruption_catalog_v1"}),
        ),
    )
...
def classify(rows: list[LedgerRow]) -> tuple[str, int | None, str, dict[str, Any]]:
    ...
    if exact:
        row = best_row(exact)
        return "exact_hit", row.event_ledger_id, "Exact volcano event found in curated official catalog.", meta
    if near:
        row = best_row(near)
        return "near_hit", row.event_ledger_id, "Near-window volcano event found in curated official catalog.", meta
    if log_only:
        row = best_row(log_only)
        return "similar_only", row.event_ledger_id, "Same volcano/event type found, but timing only partially matches.", meta
    return "miss", None, "No official catalog event matched the scoped volcano prediction.", meta
...
def main() -> int:
    ...
    try:
        with conn.cursor() as cur:
            ...
            stage3_run_id, resolved_stage3_run_key = fetch_run(cur, "stage3_event_ledger", args.stage3_run_key, family="volcano")
            source_filter = {
        ...
        if run_id is not None:
            with conn.cursor() as cur:
                update_run(cur, run_id, "failed", {"error": str(exc), "family": "volcano"})
            conn.commit()
        print(f"Stage 4 volcano run failed: {exc}", file=sys.stderr)
        return 1
    finally:
        conn.close()
