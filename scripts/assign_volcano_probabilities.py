...
SCRIPT_VERSION = "stage5_volcano_location_risk_window_v1"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Assign provisional null-model probabilities for the scoped volcano slice.")
    ...
    parser.add_argument("--run-key", help="Unique Stage 5 volcano run key. Defaults to a timestamped key.")
    parser.add_argument("--notes", default="", help="Free-form run notes.")
    parser.add_argument(
        "--baselines-path",
        default=str(REPO_ROOT / "data" / "volcano_probability_baselines.json"),
        help="Path to volcano probability baseline JSON keyed by report_number:candidate_seq.",
    )
    parser.add_argument(
        "--overrides-path",
...
def generate_run_key() -> str:
    return "stage5-volcano-" + datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
...
def insert_run(cur, run_key: str, source_filter: dict[str, Any], notes: str | None) -> int:
    cur.execute(
        ...
        (
            ...
            Json(source_filter),
            notes or None,
            Json({"family": "volcano", "model": "location_risk_window_single_event_v1"}),
        ),
    )
...
def fetch_predictions(cur, stage2_run_id: int, override_keys: list[str]) -> list[Prediction]:
    ...
    cur.execute(
        ...
        WHERE last_stage2_run_id = %s
          AND event_family_final = 'volcano'
...
def build_update(prediction: Prediction, override: dict[str, Any], baseline: dict[str, Any]) -> tuple[Any, ...]:
    ...
    return (
        ...
        p_miss,
        SCRIPT_VERSION,
        "provisional volcano null from location-specific at-risk window",
        Json(meta),
...
def main() -> int:
    ...
    try:
        with conn.cursor() as cur:
            _, resolved_stage4_run_key, stage4_filter = fetch_run(cur, "stage4_match_scoring", args.stage4_run_key, family="volcano")
            stage2_run_key = args.stage2_run_key or stage4_filter.get("stage2_run_key")
            if not stage2_run_key:
                raise RuntimeError("Could not infer Stage 2 run key from volcano Stage 4 metadata.")
        ...
        for prediction in predictions:
            ...
            if baseline is None or override is None:
                raise RuntimeError(f"Missing volcano baseline or override for {key}.")
        ...
        if not args.dry_run and run_id is not None:
            ...
            conn.commit()
        print(f"Stage 5 volcano probability run failed: {exc}", file=sys.stderr)
        return 1
    finally:
        conn.close()
