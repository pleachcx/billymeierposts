...
SCRIPT_VERSION = "stage3_politics_official_catalog_v1"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Stage 3 politics event-ledger builder from curated official events.")
    ...
    parser.add_argument("--run-key", help="Unique Stage 3 politics run key. Defaults to an auto-generated key.")
    ...
    parser.add_argument(
        "--events-path",
        default=str(REPO_ROOT / "data" / "politics_official_events.json"),
        help="Path to curated official politics events JSON.",
    )
    parser.add_argument(
        "--overrides-path",
...
def generate_run_key() -> str:
    return "stage3-politics-" + datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
...
def main() -> int:
    ...
    try:
        ...
        if run_id is not None:
            ...
            conn.commit()
        print(f"Stage 3 politics run failed: {exc}", file=sys.stderr)
        return 1
    finally:
        conn.close()
