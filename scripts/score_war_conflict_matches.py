...
SCRIPT_VERSION = "stage4_war_conflict_dated_attack_catalog_v1"
REVIEWER = "script:stage4_war_conflict_dated_attack_catalog_v1"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Stage 4 war_conflict match scorer for the dated-attack catalog.")
    ...
    parser.add_argument("--stage3-run-key", help="Stage 3 war_conflict run key. Defaults to latest completed Stage 3 war_conflict run.")
    parser.add_argument("--run-key", help="Unique Stage 4 war_conflict run key. Defaults to a timestamped key.")
...
def generate_run_key() -> str:
    return "stage4-war-conflict-" + datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
...
def classify(rows: list[LedgerRow]) -> tuple[str, int | None, str, dict[str, Any]]:
    ...
    if exact:
        row = best_row(exact)
        return "exact_hit", row.event_ledger_id, "Exact dated attack event found in the curated war_conflict catalog.", meta
    if near:
        row = best_row(near)
        return "near_hit", row.event_ledger_id, "Near-window dated attack event found in the curated war_conflict catalog.", meta
    if log_only:
        row = best_row(log_only)
        return "similar_only", row.event_ledger_id, "Same attack target cluster was found, but timing only partially matches the scoped war_conflict row.", meta
    return "miss", None, "No curated war_conflict catalog event matched the scoped attack prediction.", meta
...
def main() -> int:
    ...
    try:
        with conn.cursor() as cur:
            ...
            stage3_run_id, resolved_stage3_run_key = fetch_run(cur, "stage3_event_ledger", args.stage3_run_key, family="war_conflict")
            source_filter = {
        ...
        if run_id is not None:
            ...
            conn.commit()
        print(f"Stage 4 war_conflict run failed: {exc}", file=sys.stderr)
        return 1
    finally:
        conn.close()
