...
SCRIPT_VERSION = "stage7_war_conflict_final_v1"
REVIEWER = "script:stage7_war_conflict_final_v1"
REPO_ROOT = Path(__file__).resolve().parent.parent


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Finalize war_conflict cohort inclusion/exclusion decisions for the dated-attack slice.")
    ...
    parser.add_argument("--stage4-run-key", help="Stage 4 war_conflict run key. Defaults to latest completed Stage 4 war_conflict run.")
    parser.add_argument("--run-key", help="Unique Stage 7 war_conflict run key. Defaults to a timestamped key.")
    parser.add_argument("--notes", default="", help="Free-form run notes.")
    parser.add_argument(
        "--overrides-path",
        default=str(REPO_ROOT / "data" / "war_conflict_prediction_overrides.json"),
        help="Path to war_conflict prediction override JSON keyed by report_number:candidate_seq.",
    )
    parser.add_argument(
        "--adjudications-path",
        default=str(REPO_ROOT / "data" / "war_conflict_final_adjudications.json"),
        help="Path to war_conflict final adjudication JSON keyed by report_number:candidate_seq.",
    )
...
def generate_run_key() -> str:
    return "stage7-war-conflict-final-" + datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
...
def insert_run(cur, run_key: str, source_filter: dict[str, Any], notes: str | None) -> int:
    cur.execute(
        ...
        (
            ...
            Json(source_filter),
            notes or None,
            Json({"family": "war_conflict", "scope": "dated_attack_catalog_v1"}),
        ),
    )
...
def decide_final_status(prediction: dict[str, Any], adjudications: dict[str, dict[str, Any]]) -> tuple[str, str, dict[str, Any]]:
    ...
    if prediction["match_status"] in {"exact_hit", "near_hit", "similar_only", "miss"}:
        return (
            ...
            "Prediction belongs to the scoped war_conflict slice and has a resolved match outcome.",
            {
                "script_version": SCRIPT_VERSION,
                "reason_code": "resolved_war_conflict_slice",
            },
        )

    raise RuntimeError(f"Missing war_conflict final adjudication rule for unresolved prediction {key}.")
...
def insert_reviews(cur, run_id: int, decisions: list[tuple[int, str, str, dict[str, Any]]]) -> None:
    params = [
        (prediction_id, run_id, "war_conflict", status, True, REVIEWER, rationale, Json(meta))
...
def main() -> int:
    ...
    try:
        with conn.cursor() as cur:
            stage4_run_id, resolved_stage4_run_key, stage4_filter = fetch_run(cur, "stage4_match_scoring", args.stage4_run_key, family="war_conflict")
            stage2_run_key = args.stage2_run_key or stage4_filter.get("stage2_run_key")
            if not stage2_run_key:
                raise RuntimeError("Could not infer Stage 2 run key from Stage 4 war_conflict metadata.")
        ...
        if run_id is not None:
            with conn.cursor() as cur:
                update_run(cur, run_id, "failed", {"error": str(exc), "family": "war_conflict"})
            conn.commit()
        print(f"Stage 7 war_conflict run failed: {exc}", file=sys.stderr)
        return 1
    finally:
        conn.close()
