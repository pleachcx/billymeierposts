...
SCRIPT_VERSION = "stage5_politics_actor_risk_window_v1"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Assign provisional null-model probabilities for the scoped politics slice.")
    ...
    parser.add_argument("--run-key", help="Unique Stage 5 politics run key. Defaults to a timestamped key.")
    parser.add_argument("--notes", default="", help="Free-form run notes.")
    parser.add_argument(
        "--baselines-path",
        default=str(REPO_ROOT / "data" / "politics_probability_baselines.json"),
        help="Path to politics probability baseline JSON keyed by report_number:candidate_seq.",
    )
    parser.add_argument(
        "--overrides-path",
...
def generate_run_key() -> str:
    return "stage5-politics-" + datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
...
def insert_run(cur, run_key: str, source_filter: dict[str, Any], notes: str | None) -> int:
    cur.execute(
        ...
        (
            ...
            Json(source_filter),
            notes or None,
            Json({"family": "politics_election", "model": "actor_risk_window_single_event_v1"}),
        ),
    )
...
def build_update(prediction: Prediction, override: dict[str, Any], baseline: dict[str, Any]) -> tuple[Any, ...]:
    ...
    return (
        ...
        p_miss,
        SCRIPT_VERSION,
        "provisional politics null from actor/jurisdiction at-risk window",
        Json(meta),
...
def main() -> int:
    ...
    try:
        with conn.cursor() as cur:
            _, resolved_stage4_run_key, stage4_filter = fetch_run(cur, "stage4_match_scoring", args.stage4_run_key, family="politics_election")
            stage2_run_key = args.stage2_run_key or stage4_filter.get("stage2_run_key")
            if not stage2_run_key:
                raise RuntimeError("Could not infer Stage 2 run key from politics Stage 4 metadata.")
            ...
            if not args.dry_run:
                run_id = insert_run(
                    ...
                    run_key,
                    {
                        ...
                        "scope": "government_action_catalog_v1",
                    },
                    args.notes,
                )
                conn.commit()
        ...
        for prediction in predictions:
            ...
            if baseline is None or override is None:
                raise RuntimeError(f"Missing politics baseline or override for {key}.")
        ...
        if not args.dry_run and run_id is not None:
            ...
            conn.commit()
        print(f"Stage 5 politics probability run failed: {exc}", file=sys.stderr)
        return 1
    finally:
        conn.close()
