...
SCRIPT_VERSION = "stage4_politics_government_action_v1"
REVIEWER = "script:stage4_politics_government_action_v1"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Stage 4 politics match scorer for the government-action catalog slice.")
    ...
    parser.add_argument("--stage3-run-key", help="Stage 3 politics run key. Defaults to latest completed Stage 3 politics run.")
    parser.add_argument("--run-key", help="Unique Stage 4 politics run key. Defaults to a timestamped key.")
    parser.add_argument("--notes", default="", help="Free-form run notes.")
    parser.add_argument(
        "--overrides-path",
        default=str(REPO_ROOT / "data" / "politics_prediction_overrides.json"),
        help="Path to politics prediction override JSON keyed by report_number:candidate_seq.",
    )
...
def generate_run_key() -> str:
    return "stage4-politics-" + datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
...
def insert_run(cur, run_key: str, source_filter: dict[str, Any], notes: str | None) -> int:
    cur.execute(
        ...
        (
            ...
            Json(source_filter),
            notes or None,
            Json({"family": "politics_election", "scope": "government_action_catalog_v1"}),
        ),
    )
...
def classify(rows: list[LedgerRow]) -> tuple[str, int | None, str, dict[str, Any]]:
    ...
    if exact:
        row = best_row(exact)
        return "exact_hit", row.event_ledger_id, "Exact government-action event found in curated official catalog.", meta
    if near:
        row = best_row(near)
        return "near_hit", row.event_ledger_id, "Near-window government-action event found in curated official catalog.", meta
    if log_only:
        row = best_row(log_only)
        return "similar_only", row.event_ledger_id, "Same actor/action government event found, but timing only partially matches.", meta
    return "miss", None, "No official catalog event matched the scoped politics prediction.", meta
...
def main() -> int:
    ...
    try:
        with conn.cursor() as cur:
            ...
            stage3_run_id, resolved_stage3_run_key = fetch_run(cur, "stage3_event_ledger", args.stage3_run_key, family="politics_election")
            source_filter = {
                "stage2_run_key": resolved_stage2_run_key,
                "stage3_run_key": resolved_stage3_run_key,
                "family": "politics_election",
        ...
        if run_id is not None:
            with conn.cursor() as cur:
                update_run(cur, run_id, "failed", {"error": str(exc), "family": "politics_election"})
            conn.commit()
        print(f"Stage 4 politics run failed: {exc}", file=sys.stderr)
        return 1
    finally:
        conn.close()
